          ...
          <scroll class="middle-r" ref="lyricList" :data="currentLyric && currentLyric.lines">
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p ref="lyricLine" class="text" :class="{'current': currentLineNum === index}" :key="line.time" v-for="(line, index) in currentLyric.lines">
                  {{line.txt}}
                </p>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    updateTime(e) {
      // e.target.currentTime 是一个可读写的属性，通过写入新值，可以控制播放的位置
    ...
    _getPosAndScale() {
      const targetWitdh = 40
      const paddingLeft = 40
      const paddingBottom = 30
      const paddingTop = 80
      const width = window.innerWidth * 0.8
      const scale = targetWitdh / width
